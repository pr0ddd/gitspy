...
describe('badges on chips', () => {
  it('marks a local branch with the laptop glyph, and it sits right of the name as in GitKraken', () => {
  ...
  it('a branch with an open pull request carries its badge last in the tail', () => {
    ...
    expect(pull, 'an open pull request is marked with a badge, as in GitKraken').toBeDefined();
  ...
  it('the working tree row counts files by fate: modified, added, deleted', () => {
    ...
    expect(
      ...
      'the pencil marks modified files, as in GitKraken',
    ).toBe(true);
    expect(

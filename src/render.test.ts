...
describe('метки на чипах', () => {
  it('локальная ветка несёт ноутбук, и он стоит справа от имени, как в GitKraken', () => {
  ...
  it('ветка с открытым пул-реквестом несёт его значок последним в хвосте', () => {
    ...
    expect(pull, 'открытый PR подписывается значком, как в GitKraken').toBeDefined();
  ...
  it('строка рабочего дерева считает файлы по судьбе: изменено, добавлено, удалено', () => {
    ...
    expect(
      ...
      'карандаш — знак изменённых, как в GitKraken',
    ).toBe(true);
    expect(

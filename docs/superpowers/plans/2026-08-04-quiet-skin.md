...
### Task 6: Граф — чистая часть — ОТМЕНЕНО

Заказчик остановил графовые задачи после задачи 5: концепт проверяется
на оболочке, граф получает только новые цвета через токены. Задача 6
была выполнена и откачена ревертом `f889fdd`; задача 7 не начиналась.
Эталон вида на будущее — linear.app.
...
### Task 7: Граф — краска — ОТМЕНЕНО (см. задачу 6)
